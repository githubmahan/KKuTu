...
$audiosets = [
	[
		{
			key: "lobby",
			value: "/media/LobbyBGM.mp3"
		},
		{
			key: "lobby4fool",
			value: "/media/revLobbyBGM.mp3"
		},
		{
			key: "lobby1",
			value: "/media/LobbyBGM1.mp3"
		},
		{
			key: "lobby2",
			value: "/media/LobbyBGM2.mp3"
		},
		{
			key: "lobby3",
...
for (i = 0; i <= 10; i++) $audiosets[1].push(
	{
		key: "T" + i,
		value: "/media/mahankkutu/T" + i + ".mp3"
	},
	{
		key: "K" + i,
		value: "/media/kkutu/K" + i + ".mp3"
...
$(document).ready(function(){
	...
	loadSounds($audiosets[$("#effect-select").val()], function(){
		processShop(connect);
	});
...
// 메뉴 버튼
	...
	var playtime = 1;
	_setInterval(() => {notice(`게임을 플레이한지 ${playtime}시간이 지났습니다. 과도한 게임 이용은 일상생활에 지장을 줄 수 있습니다. 본 게임물은 전체이용가입니다.`); playtime += 1}, 3600000)
...
// 웹소켓 연결
	function connect(){
		...
		ws.onopen = function(e){
			loading();
			_setInterval(() => ws.send('연결용'), 10000);		
...
$lib.Free.roundReady = $lib.Daneo.roundReady
...
function addonNickname($R, o){
	...
	if(o.equip['Mbadge'] == "gm_gm") $R.addClass("x-gm");
	if(o.equip['Mbadge'] == "gm_sgm") $R.addClass("x-sgm");
	if(o.equip['Mbadge'] == "gm_dgm") $R.addClass("x-dgm");
	if(o.equip['Mbadge'] == "gm_magm") $R.addClass("x-magm");
	if(o.equip['Mbadge'] == "gm_wgm") $R.addClass("x-wgm");
	if(o.equip['Mbadge'] == "gm_pgm") $R.addClass("x-pgm");
	if(o.equip['Mbadge'] == "gm_mgm") $R.addClass("x-mgm");
	if(o.equip['Mbadge'] == "gm_mkgm") $R.addClass("x-mkgm");
}
...
function requestProfile(id){
	...
	if(o.robot){
		...
		for(i in o.data.record){
			...
			var winPercentage = r[0] > 0 ? Math.round((r[1] / r[0]) * 100) : 0;
			
			$rec.append($("<div>").addClass("profile-record-field")
				.append($("<div>").addClass("profile-field-name").html(L['mode' + i]))
				.append($("<div>").addClass("profile-field-record").html(r[0] + L['P'] + " " + r[1] + L['W'] + " (" + winPercentage + "%)"))
	...
	if($data.id == id) {$stage.dialog.profileDress.show(); $stage.dialog.profileFriend.hide();}
	else if(!o.robot){
...
function playBGM(key){
	...
	return $data.bgm = playSound(key === "lobby" ? $("#bgm-select").val() : key, true)
}
function stopBGM(){
	if($data.bgm){
		$data.bgm.stop();
...
function iImage(key, sObj){
	...
	if(obj.group.slice(0, 3) == "BDG") return "/img/kkutu/moremi/insignia/" + obj._id + gif;
	if(obj._id.slice(0, 2) == "gm") return "/img/kkutu/moremi/badge/gm_gm" + gif;
...
function renderMoremi(target, equip){
	...
	// 	.attr('src', equip.robot ? "/img/kkutu/moremi/skin/robot.png" : "/img/kkutu/moremi/body.png")
	// 	.css({ 'width': "100%", 'height': "100%" })
	// );
