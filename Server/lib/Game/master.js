	...
	switch(cmd){
		...
		case 'ban':
			try {
				...
				if(DIC[id]) DIC[id].send('block', `[Block] 사용자 #${args[0].trim()}(이)가 이용제한 처리되었습니다.`);
		...
		case 'ipban':
			try {
				...
				if(DIC[id]) DIC[id].send('block', `[Block] IP 주소 ${args[0].trim()}(이)가 이용제한 처리되었습니다.`);
			}catch(e){
				processAdminErrorCallback(e, id);
			}
			return null;
		case 'unban':
			try {
				...
				if(DIC[id]) DIC[id].send('block', `사용자 #${value}(이)가 이용제한 해제 처리되었습니다.`);
			}catch(e){
				processAdminErrorCallback(e, id);
			}
			return null;
		case 'ipunban':
			try {
				...
				if(DIC[id]) DIC[id].send('block', `IP 주소 ${value}(이)가 이용제한 해제 처리되었습니다.`);
			}catch(e){
				processAdminErrorCallback(e, id);
			}
			return null;
	...
	MainDB.ready = function(){
		...
		Server.on('connection', function(socket, info){
			...
			MainDB.session.findOne([ '_id', key ]).limit([ 'profile', true ]).on(function($body){
				...
				/* Toggle Guest Entrance & User Kick [S] */
				if (!GLOBAL.ALLOWENTER_OPTIONS.ALLOWENTER && !($c.admin || $c.tester)) {
					$c.sendError(500);
					$c.socket.close();
					return;
				}
				...
				if($c.guest){
					...
					/* Toggle Guest Entrance & User Kick [S] */
					if (!GLOBAL.ALLOWENTER_OPTIONS.ALLOWGUESTENTER) {
						$c.sendError(458);
						$c.socket.close();
						return;
					}
				...
				$c.refresh().then(function(ref){
					...
					/* Enhanced User Block System [E] */
						...
						if (($c.guest && GLOBAL.RECAPTCHA_OPTIONS.TO_GUEST) || GLOBAL.RECAPTCHA_OPTIONS.TO_USER) {
							$c.socket.send(JSON.stringify({
								type: 'recaptcha',
								siteKey: GLOBAL.RECAPTCHA_OPTIONS.SITE_KEY
							}));
						} else {
	...
	switch (msg.type) {
		...
		case 'setRoom':
			...
			if (stable) {
				...
				const { unknownword, randomturn, returns, mission, sami, ...optstemp} = msg.opts
				if (msg.opts.unknownword && Object.values(optstemp).some(value => value === true)) {msg.code = 600; stable = false;}
				if (msg.opts.unlimited && msg.opts.short) {msg.code = 601; stable = false;}
				// if (msg.mode === 14 || msg.mode === 15) {{msg.code = 602; stable = false;}} // 개발중인 모드
			}
			if (msg.type == 'enter') {
